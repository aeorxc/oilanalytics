...
def get_file_url():
    html_page = urllib.request.urlopen("https://www.opec.org/opec_web/en/publications/338.htm")
    soup = BeautifulSoup(html_page, "html.parser")
    links = [x.get('href') for x in soup.findAll('a') if x.get('href').endswith('.xlsx')]
    link = links[0]
