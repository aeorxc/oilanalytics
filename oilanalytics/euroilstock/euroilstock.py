...
rics = {'CFSL-EU-EBBL': 'Crude Stocks',
...
def generate_euroil_stock_page(out_loc=None):
    ...
    df = ek.get_timeseries(list(rics.keys()), fields='Close', start_date=start_date).rename(columns=rics)
    ...
    ju.render_html(data, 'euroilstock.html', filename=out_loc, package_loader_name='oilanalytics.euroilstock')
