...
def generate_oi_vol_plot(df: pd.DataFrame):
    ...
    fig = make_subplots(rows=3, cols=1, shared_xaxes=True,
                        vertical_spacing=0.03, subplot_titles=('Price', 'Volume', 'OpenInterest'),
...
def generate_symbol_page(limsymbol: str, symbolname: str, cust_charts: dict = None, file_path: str = None):
    df = lim.series(limsymbol, start_date=pd.to_datetime('2015'))
    candle = lim.candlestick_data(limsymbol, additional_columns=('TotalOI', 'TotalVol'), days=440)
    ...
    for spread in ['janfeb', 'febmar', 'marapr', 'aprmay', 'mayjun', 'junjul', 'julaug', 'augsep', 'sepoct', 'octnov',
                   'novdec', 'decjan']:
    ...
    return ju.render_html(data, 'symbol.html', filename=file_path, package_loader_name='oilanalytics.prices')
...
def generate_structure_page(start_date: t.Optional[t.Union[str, date]] = date(2018, 1, 1),
    ...
    return ju.render_html(data, 'structure.html', filename=file_path, package_loader_name='oilanalytics.prices')
